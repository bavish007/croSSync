...
const localSetupInit = {
  ...
  init() {
    ...
    this.OAuthClientSecret = (window.croSSyncConfig || {}).OAuthClientSecret;
  ...
  getGithubToken(githubAccessCode) {
    const that = this;
    const githubUserToken = this.githubUserToken;
    ...
    xhttp.addEventListener('readystatechange', function () {
      if (xhttp.readyState === 4) {
        if (xhttp.status === 200) {
          that.sendMessageLocalSetup(xhttp.responseText.match(/access_token=([^&]*)/)[1], githubUserToken);
        } 
        else {
  ...
  sendMessageLocalSetup(accessToken, githubUserToken) {
